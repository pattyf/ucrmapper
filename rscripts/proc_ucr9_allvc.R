...
#
#murderCost <- 7,000,000
#rapeCost <- 142,020
#robberyCost <- 12,624
#assaultCost <- 38,924
city_vc$cw_vc <- (city_vc$murder_tot * 7000000) + (city_vc$rape_tot * 7000000) + (city_vc$rob_tot * 12624 ) + (city_vc$agg_tot * 38924 )
...
city50k_cwvc <- city_vc_lonlat[,c(keep_cols,"cw_vc")]
...
filez = list.files(pattern=".geojson")
sapply(filez,FUN=function(eachPath){
 file.rename(from=eachPath,to=sub(pattern="geojson",replacement="js",eachPath))
})
...
# So that the json data values will be set to and accessible via these variable names.
## IMPORTANT - THIS NEEDS TO BE DONE MANUALLLY!!!
